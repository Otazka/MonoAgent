...
class DependencyGraphVisualizer:
        ...
        for project_name, project in projects.items():
            for dep in project.dependencies:
                if dep in projects or dep in common_components:
                    self.graph.add_edge(project_name, dep, type="dependency", weight=1)
        ...
        for conflict in conflicts:
            if conflict.conflict_type in ["missing_dependency", "circular_dependency"]:
                ...
                if self.graph.has_edge(conflict.source_project, conflict.target_project):
                    ...
                    self.graph[conflict.source_project][conflict.target_project][
                        "conflict_type"
    ...
    def _draw_nodes(self, pos):
        ...
        component_nodes = [
            n for n, d in self.graph.nodes(data=True) if d.get("type") == "component"
        ]
    ...
    def _draw_edges(self, pos):
        ...
        # Normal edges
        normal_edges = [
            (u, v) for u, v in self.graph.edges() if not self.graph[u][v].get("conflict", False)
        ]
        ...
        conflict_edges = [
            (u, v) for u, v in self.graph.edges() if self.graph[u][v].get("conflict", False)
        ]
        nx.draw_networkx_edges(
            self.graph,
            pos,
    ...
    def _add_legend(self):
        ...
        legend_elements.append(
            plt.Line2D([0], [0], color="#666666", lw=2, label="Normal Dependency")
        )
        legend_elements.append(
            plt.Line2D([0], [0], color="#FF4444", lw=3, label="Conflict Dependency")
        )
...
class MonorepoAnalyzer:
    ...
    def analyze_repository_structure(
        self,
    ) -> Tuple[Dict[str, ProjectInfo], Dict[str, CommonComponent]]:
                ...
                for file_name in files:
                    if file_name in project_indicators:
                        ...
                        if project_name not in self.projects:
                            ...
                            self.logger.info(
                                f"  ✅ Detected {project_type} project: {project_name} at {dir_path}"
                            )
                        break
                pbar.update(1)
    ...
    def _detect_by_directory_structure(self, all_files: List[str]):
        ...
        with tqdm(
            total=len(all_files), desc="🏗️  Analyzing directory structure", unit="file"
        ) as pbar:
            for file_path in all_files:
                    ...
                    if match:
                        ...
                        if project_name not in self.projects:
                            ...
                            if self._is_substantial_project(dir_path, all_files):
                                ...
                                self.logger.info(
                                    f"  ✅ Detected app project: {project_name} at {dir_path}"
                                )
                        break
                pbar.update(1)
    ...
    def _is_substantial_project(self, dir_path: str, all_files: List[str]) -> bool:
        ...
        source_files = [
            f for f in project_files if any(f.endswith(ext) for ext in source_extensions)
        ]

        # Consider it substantial if it has multiple source files or config files
        return len(source_files) > 3 or any(
            f.endswith((".json", ".yaml", ".yml", ".toml", ".xml")) for f in project_files
        )
                    ...
                    if match:
                        ...
                        if component_name not in self.common_components:
                            ...
                            self.common_components[component_name] = CommonComponent(
                                name=component_name, path=dir_path, files=component_files
                            )
                            self.logger.info(
                                f"  ✅ Detected common component: {component_name} at {dir_path}"
                            )
                        break
                pbar.update(1)
        ...
        total_files = sum(
            len(
                [
                    ...
                    if f.endswith((".js", ".ts", ".jsx", ".tsx", ".py", ".java", ".go"))
                ]
            )
    ...
    def _generate_analysis_report(self):
        ...
        # Build dependency graph (used for analysis)
        visualizer.build_graph(self.projects, self.common_components, self.conflicts)
        ...
        if len(self.projects) > 1:
            recommendations.append(
                f"Split {len(self.projects)} detected projects into separate repositories"
            )

        if self.common_components:
            recommendations.append(
                f"Extract {len(self.common_components)} common components into shared libraries"
            )
        ...
        if critical_conflicts:
            recommendations.append(
                f"⚠️  Resolve {len(critical_conflicts)} critical dependency conflicts before splitting"
            )

        if high_conflicts:
            recommendations.append(
                f"⚠️  Address {len(high_conflicts)} high-severity dependency conflicts"
            )
        ...
        if self.conflicts:
            ...
            for conflict in self.conflicts:
                ...
                self.logger.info(
                    f"  {severity_emoji} {conflict.conflict_type.upper()}: {conflict.description}"
                )
...
class RepoSplitter:
    ...
    # -------------------------------
    def _log_issue(
        self, level: str, component: str, context: str, message: str, cause: str, fix: str
    ) -> None:
    ...
    def preflight_checks(self) -> bool:
        ...
        if self.config.visualize and shutil.which("dot") is None:
            self.logger.warning(
                "Graphviz 'dot' not found. Visualizations (PNG/SVG) will be skipped."
            )

        # SOURCE_REPO_URL reachability
        try:
            self.run_git_command(
                ["git", "ls-remote", "--heads", self.config.source_repo_url], check=False
            )
        except Exception:
            self.logger.warning(
                "Could not verify SOURCE_REPO_URL reachability. Ensure the URL and SSH agent are configured."
            )
        ...
        try:
            if provider == "github":
                ...
                resp = requests.get(
                    f"{base}/api/v4/user",
                    headers={"PRIVATE-TOKEN": self.config.github_token},
                    timeout=10,
                )
                ...
                if resp.status_code != 200:
                    self.logger.error(
                        f"Bitbucket token check failed: {resp.status_code} {resp.text}"
                    )
                    ok = False
            elif provider == "azure":
                import base64
                ...
                if resp.status_code != 200:
                    self.logger.error(
                        f"Azure DevOps PAT check failed: {resp.status_code} {resp.text}"
                    )
                    ok = False
        except Exception as e:
    ...
    def load_config(self) -> RepoSplitterConfig:
        ...
        config = RepoSplitterConfig(
            ...
            manual_projects=(
                os.getenv("MANUAL_PROJECTS", "").split(",")
                if os.getenv("MANUAL_PROJECTS")
                else ([p.strip() for p in env_projects.split(",")] if env_projects else None)
            ),
            manual_common_paths=(
                os.getenv("MANUAL_COMMON_PATHS", "").split(",")
                if os.getenv("MANUAL_COMMON_PATHS")
                else None
            ),
            exclude_patterns=(
                os.getenv("EXCLUDE_PATTERNS", "").split(",")
                if os.getenv("EXCLUDE_PATTERNS")
                else None
            ),
            mode=env_mode,
    ...
    # ----------------------------
    @staticmethod
    def _transform_package_json(
        original: Dict, *, repo_name: str, repo_url: str, is_library: bool
    ) -> Dict:
        ...
        # Repository metadata
        repo_meta = {"type": "git", "url": repo_url}
    ...
    def _migrate_node_project(
        self, package_json_path: str, *, repo_name: str, repo_url: str, is_library: bool
    ) -> None:
        ...
        migrated = self._transform_package_json(
            data, repo_name=repo_name, repo_url=repo_url, is_library=is_library
        )
    ...
    def run_git_command(
        self, command: List[str], cwd: str = None, check: bool = True
    ...
    def _rate_limit_guard(self) -> None:
        ...
        try:
            ...
            if (
                ...
                and remaining <= self.config.rate_limit_min_remaining
            ):
                now = datetime.now(timezone.utc)
                wait_seconds = max((reset - now).total_seconds(), 0) + 1
                self.logger.warning(
                    f"⏳ GitHub rate limit low (remaining={remaining}). Waiting {int(wait_seconds)}s until reset..."
                )
    ...
    def _handle_github_exception_backoff(self, exc: GithubException) -> None:
        ...
        if retry_after is None:
            try:
                ...
                now = datetime.now(timezone.utc)
                retry_after = max(
                    int((rl.reset - now).total_seconds()) + 1, self.config.abuse_retry_after_default
                )
            except Exception:
    ...
    def clone_source_repo(self) -> str:
        ...
        try:
            ...
            if free_gb < self.config.min_free_gb:
                raise RuntimeError(
                    f"Insufficient disk space: {free_gb:.2f}GB < {self.config.min_free_gb}GB"
                )
        except Exception as e:
            self._log_issue(
                "error",
                ...
                str(e),
                "Free up space or change temp directory",
            )
            raise
    ...
    def analyze_monorepo(self) -> Tuple[Dict[str, ProjectInfo], Dict[str, CommonComponent]]:
        ...
        if critical_conflicts and not self.config.dry_run:
            self.logger.warning(
                f"⚠️  Found {len(critical_conflicts)} critical dependency conflicts!"
            )
            self.logger.warning(
                "Consider resolving these conflicts before proceeding with the split."
            )

            # Ask for confirmation in non-dry-run mode
            if not self.config.force:
                self.logger.error(
                    "❌ Aborting due to critical dependency conflicts. Use --force to proceed anyway."
                )
                raise ValueError(
                    f"Critical dependency conflicts detected: {len(critical_conflicts)} conflicts"
                )

        return projects, common_components

    def _create_repo_provider_agnostic(
        self, repo_name: str, description: str = ""
    ) -> Optional[str]:
            ...
            for attempt in range(1, 4):
                try:
                    ...
                    try:
                        ...
                        self.logger.warning(
                            f"Repository {repo_name} already exists, skipping creation"
                        )
                        pbar.update(1)
                    ...
                    last_exc = e
                    self.logger.warning(
                        f"GitHub API error creating {repo_name} (attempt {attempt}/3): {e}"
                    )
                    if (
                        getattr(e, "status", None) == 403
                        or "rate limit" in str(e).lower()
                        or "abuse" in str(e).lower()
                    ):
        ...
        try:
            ...
            if resp.status_code in (200, 201):
                ...
                self._log_issue(
                    ...
                    resp.text[:200],
                    "Verify token and permissions; ensure project path unique",
                )
        except Exception as e:
            self._log_issue(
                "error",
                ...
                str(e),
                "Check network and GITLAB_HOST; retry later",
            )
        return None
    ...
    def _create_repo_bitbucket(self, repo_name: str, description: str = "") -> Optional[str]:
        ...
        try:
            self._rate_limit_guard()
            resp = requests.post(
                f"{base}/repositories/{user}/{name}", auth=auth, json=data, timeout=30
            )
            if resp.status_code in (200, 201):
                info = resp.json()
                ...
                self._log_issue(
                    ...
                    f"API error {resp.status_code}",
                    resp.text[:200],
                    "Verify app password, username, and workspace",
                )
        except Exception as e:
            self._log_issue(
                "error",
                ...
                str(e),
                "Check network, auth, and repository name",
            )
        return None
    ...
    def _create_repo_azure(self, repo_name: str, description: str = "") -> Optional[str]:
        ...
        if not project:
            self._log_issue(
                ...
                "azure",
                "create_repo",
                "azure_project missing",
                "config.azure_project not set",
                "Set AZURE_PROJECT in env or CLI",
            )
            return None
        # user = self.config.provider_username or ""  # Not used in Azure API
        ...
        try:
            ...
            if resp.status_code in (200, 201):
                ...
                self._log_issue(
                    ...
                    f"API error {resp.status_code}",
                    resp.text[:200],
                    "Verify PAT permissions and org/project names",
                )
        except Exception as e:
            self._log_issue(
                "error",
                ...
                "Repo creation failed",
                str(e),
                "Check network and PAT; retry later",
            )
        return None
    ...
    def extract_project_to_repo(self, project: ProjectInfo, repo_name: str, repo_url: str):
        ...
        if not self.config.dry_run:
            with tqdm(total=7, desc=f"📦 Extracting {project.name}", unit="step") as pbar:
                ...
                try:
                    self._migrate_node_project(
                        os.path.join(project_repo_path, "package.json"),
                        repo_name=repo_name,
                        repo_url=repo_url,
                        is_library=False,
                    )
                ...
                # Add new remote
                self.run_git_command(
                    ["git", "remote", "add", "origin", repo_url], cwd=project_repo_path
                )
                pbar.update(1)

                # Ensure default branch name
                self.run_git_command(
                    ["git", "branch", "-M", self.config.default_branch], cwd=project_repo_path
                )
                pbar.update(1)

                # Push to the new repository
                self.run_git_command(
                    ["git", "push", "-u", "origin", self.config.default_branch],
                    cwd=project_repo_path,
                )
                pbar.update(1)
    ...
    def extract_common_component_to_repo(
        self, component: CommonComponent, repo_name: str, repo_url: str
    ):
        ...
        self.logger.info(
            f"Extracting common component '{component.name}' to repository '{repo_name}'"
        )
        ...
        if not self.config.dry_run:
            with tqdm(total=7, desc=f"🔧 Extracting {component.name}", unit="step") as pbar:
                ...
                if not os.path.exists(target_path):
                    self.logger.error(
                        f"Common component path does not exist in repo: {component.path}"
                    )
                    return
                pbar.update(1)
                ...
                try:
                    self._migrate_node_project(
                        os.path.join(component_repo_path, "package.json"),
                        repo_name=repo_name,
                        repo_url=repo_url,
                        is_library=True,
                    )
                ...
                # Add new remote
                self.run_git_command(
                    ["git", "remote", "add", "origin", repo_url], cwd=component_repo_path
                )
                pbar.update(1)

                # Ensure default branch name
                self.run_git_command(
                    ["git", "branch", "-M", self.config.default_branch], cwd=component_repo_path
                )
                pbar.update(1)
            ...
            self.logger.info(
                f"Successfully extracted common component '{component.name}' to '{repo_name}'"
            )
        ...
        if not self.config.dry_run:
            with tqdm(total=8, desc=f"🌿 Extracting {branch_name}", unit="step") as pbar:
                ...
                try:
                    ...
                    self.run_git_command(
                        ["git", "checkout", f"origin/{branch_name}"], cwd=branch_repo_path
                    )
                    self.run_git_command(
                ...
                self.run_git_command(
                    ["git", "branch", "-M", self.config.default_branch], cwd=branch_repo_path
                )
                pbar.update(1)
                ...
                self.run_git_command(
                    ["git", "remote", "add", "origin", repo_url], cwd=branch_repo_path
                )
                pbar.update(1)
        ...
        try:
            ...
            if mode == "auto":
                ...
                if self.config.analyze_only:
                    self.logger.info(
                        "Analysis complete. Use --dry-run to see what would be created."
                    )
                    return
                ...
                with tqdm(total=total_items, desc="🚀 Creating repositories", unit="repo") as pbar:
                    ...
                    # tasks = []  # Not used in current implementation

                    def submit_project(project):
                        repo_name = self._sanitize_repo_name(
                            self.config.repo_name_template_app.format(name=project.name)
                        )
                    ...
                    def submit_component(component):
                        repo_name = self._sanitize_repo_name(
                            self.config.repo_name_template_lib.format(name=component.name)
                        )
                ...
                if not projects_list:
                    raise ValueError(
                        "PROJECTS (or MANUAL_PROJECTS) must be provided in project mode"
                    )

                total_items = len(projects_list) + (1 if self.config.common_path else 0)
                with tqdm(
                    total=total_items, desc="🚀 Creating project repositories", unit="repo"
                ) as pbar:
                    for project_path in projects_list:
                        ...
                        repo_name = self._sanitize_repo_name(
                            self.config.repo_name_template_app.format(name=project_name)
                        )
                        ...
                        if repo_url:
                            ...
                            self.logger.error(
                                f"Failed to create repository for project: {project.name}"
                            )
                        pbar.update(1)
                    ...
                    if self.config.common_path:
                        ...
                        repo_name = self._sanitize_repo_name(
                            self.config.repo_name_template_lib.format(name=comp_name)
                        )
                        description = "Common library component extracted from monorepo"
                        ...
                        if repo_url:
                            ...
                            self.logger.error(
                                f"Failed to create repository for common component: {component.name}"
                            )
                        pbar.update(1)
                ...
                with tqdm(
                    total=len(branches), desc="🌿 Creating branch repositories", unit="branch"
                ) as pbar:
                    for branch_name in branches:
                        ...
                        if not name_clean:
                            continue
                        repo_name = self._sanitize_repo_name(
                            self.config.repo_name_template_app.format(name=name_clean)
                        )
                        ...
                        if repo_url:
                            ...
                            self.logger.error(
                                f"Failed to create repository for branch: {name_clean}"
                            )
                        pbar.update(1)
...
def main():
    ...
    parser.add_argument(
        "--analyze-only",
        action="store_true",
        help="Only analyze the monorepo structure without splitting",
    )
    parser.add_argument(
        "--force", action="store_true", help="Force proceed despite dependency conflicts"
    )
    parser.add_argument(
        "--visualize", action="store_true", help="Generate dependency graph visualizations"
    )
    parser.add_argument("--preflight", action="store_true", help="Run preflight checks and exit")
    # Universal CLI options
    parser.add_argument(
        "--mode", choices=["auto", "project", "branch"], help="Splitting mode to use"
    )
    parser.add_argument("--branches", help="Comma-separated list of branches for branch mode")
    parser.add_argument(
        "--projects", help="Comma-separated list of project directories for project mode"
    )
    ...
    parser.add_argument(
        "--default-branch", help="Default branch name for created repositories (e.g., main, master)"
    )
    ...
    try:
        ...
        config = RepoSplitterConfig(
            ...
            repo_name_template_app=(
                args.name_template_app if args.name_template_app else "{name}-app"
            ),
            repo_name_template_lib=(
                args.name_template_lib if args.name_template_lib else "{name}-lib"
            ),
        )
